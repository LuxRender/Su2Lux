...
class LuxrenderSettings
    ...
    @@settings_interface = @@settings_render.merge(@@settings_scene)

	@@settings = @@settings_interface.merge(@@settings_other)
	...
	def initialize
		...
		singleton_class.module_eval do
			...
			@@settings.each do |key, value|
				######## -- create methods to access any attribute -- #######
				...
				case key
					when LuxrenderSettings::ui_refreshable?(key)# set ui_refreshable
						...
						define_method("#{key}=") { |new_value| @attributedictionary.set_attribute(@dict, key, new_value) }
				end #end case
	...
	def reset
			...
			@attributedictionary.set_attribute(@dict, 'fov', format("%.2f", @model.active_view.camera.fov))
			@attributedictionary.set_attribute(@dict, 'focal_length', format("%.2f", @model.active_view.camera.focal_length))
			@attributedictionary.set_attribute(@dict, 'fleximage_xresolution', @model.active_view.vpwidth)
			@attributedictionary.set_attribute(@dict, 'fleximage_yresolution', @model.active_view.vpheight)
	end #END reset
