...
class LuxrenderSettings
	...
	def initialize(modelID)
		...
		@settings.each do |key, defaultValue|
			...
			define_singleton_method(key) { @attributedictionary.get_attribute("luxrender_settings", key, defaultValue) }
			# create setter methods for all parameters
			if(LuxrenderSettings::ui_refreshable?(key))# create setter methods for path fields
				...
				define_singleton_method("#{key}=") { |new_value| @attributedictionary.set_attribute("luxrender_settings", key, new_value) }
			end #end case
